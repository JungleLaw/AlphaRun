...
import cn.alpha.conf.AppConf;
import cn.alpha.utils.AppUtils;
import cn.alpha.utils.EncryptUtils;
...
public class DiskCache implements ICache {
    ...
    private DiskCache(Context context) {
        ...
        try {
            ...
            cache = DiskLruCache.open(cacheDir, AppUtils.getAppInfo(context).getVersionCode(), 1, 10 * 1024 * 1024);        //10M
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static String getMd5Key(String key) {
        return EncryptUtils.encryptMD5ToString(key.getBytes());
    }
